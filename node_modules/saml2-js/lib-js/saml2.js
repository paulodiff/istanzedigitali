...
// PATCH SingleLogoutService
create_metadata = function(entity_id, assert_endpoint, single_logout_service, signing_certificates, encryption_certificates) {
//create_metadata = function(entity_id, assert_endpoint, signing_certificates, encryption_certificates) {

  console.log('PATCH : create_metadata');
  console.log(single_logout_service);
      ...
      'md:SPSSODescriptor': [].concat({
      ...
      }).concat(signing_cert_descriptors).concat(encryption_cert_descriptors).concat([
        {
          'md:SingleLogoutService': {
            ...
            '@Location': single_logout_service
          },
...
sign_request = function(saml_request, private_key, relay_state, response) {
  ...
  var URL_ENCODE_ALG = "http://www.w3.org/2000/09/xmldsig#rsa-sha1";
  var ENCODE_ALG = "RSA-SHA1";

  // var URL_ENCODE_ALG = 'http://www.w3.org/2001/04/xmldsig-more#rsa-sha256';
  // var ENCODE_ALG = "RSA-SHA256";
  ...
  data += "&SigAlg=" + encodeURIComponent(URL_ENCODE_ALG);
  ...
  sigalg_data = "&SigAlg=" + encodeURIComponent(URL_ENCODE_ALG);

  console.log('## Returns an array of the names of the supported hash algorithms');
  const hashes = crypto.getHashes();
  console.log(hashes); // ['DSA', 'DSA-SHA', 'DSA-SHA1', ...]

  sign = crypto.createSign(ENCODE_ALG);
  ...
  samlQueryString.SigAlg = URL_ENCODE_ALG;
...
check_saml_signature = function(xml, certificate) {
  ...
  console.log('\ncheck_saml_signature##START############################');

  console.log('\ncheck_saml_signature:certificate');
  ...
  console.log('\ncheck_saml_signature:xml --------------------------------------------------');
  console.log(xml);


  console.log('\ncheck_saml_signature:signature --------------------------------------------------');
  console.log(signature);

  console.log('\ncheck_saml_signature:signature 0 --------------------------------------------------');
  console.log(signature[0].toString());


  var myCert =  format_pem(certificate, 'CERTIFICATE');
  console.log('\ncheck_saml_signature.myCert --------------------------------------------------');
  console.log(myCert);
  ...
  console.log('\ncheck_saml_signature.valid:',valid);
  
  if (valid) {
    console.log('\ncheck_saml_signature.call get_signed_data ...');
    return get_signed_data(doc, sig.references);
  } else {
    console.log('\ncheck_saml_signature.validationErrors --------------------------------------------------');
...
get_attribute_value = function(node, attributeName) {
  ...
  console.log('\nget_attribute_value:attributes');
...
get_session_info = function(dom, index_required) {
  ...
  if (assertion.length > 1) {
    console.log('get_session_info:WARNING Assertion found', assertion.length);
  }
  ...
  console.log('\nget_session_info:authn_statement[0]');
  console.log(authn_statement[0]);
  ...
  return async.waterfall([
    function(cb_wf) {
      return decrypt_assertion(saml_response, sp_private_keys, function(err, result) {
        ...
        // PATCH Assertion in Advice 
        console.log(XMLNS.SAML);
        console.log(assertion.length);
        console.log(assertion[0].toString().length);
        console.log(assertion[1].toString().length);
      ...
      console.log('\nparse_authn_response:ignore_signature',ignore_signature);
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        ...
        console.log('\nparse_authn_response:signed_data');
        ...
        for (_j = 0, _len1 = signed_data.length; _j < _len1; _j++) {
          ...
          console.log('\nparse_authn_response:signed_data:', _j);
          sd = signed_data[_j];
          //console.log('\nparse_authn_response:sd:',sd);
          signed_dom = (new xmldom.DOMParser()).parseFromString(sd);
          console.log('\nparse_authn_response:signed_dom:',signed_dom);
          assertion = signed_dom.getElementsByTagNameNS(XMLNS.SAML, 'Assertion');
          
          console.log('\nparse_authn_response:assertion:',assertion);
          console.log('\nparse_authn_response:assertion.length:',assertion.length);
          // PATCH if (assertion.length === 1) diventa > 1 (Tag Advice)
          if (assertion.length >= 1) {
      ...
      console.log('\nparse_authn_response:decrypted_assertion:');
      ...
      try {
        ...
        console.log('\nparse_authn_response:get_sessuib_info');
        console.log(decrypted_assertion);
        console.log(require_session_index);
...
module.exports.ServiceProvider = ServiceProvider = (function() {
  function ServiceProvider(options) {
    ...
    // PATCH single_logout_service
    this.single_logout_service = options.single_logout_service;
    ...
    return zlib.deflateRaw(xml, (function(_this) {
      return function(err, deflated) {
        ...
        if (options.sign_get_request) {
          ...
          console.log('create_login_request_url');
          console.log(options.relay_state);
  ...
  ServiceProvider.prototype.create_metadata = function() {
    ...
    return create_metadata(this.entity_id, this.assert_endpoint, this.single_logout_service, certs, certs);
  };
