...
const serviceWorkerVersion = 'Service Worker version 1.0';
...
function demoMustShowNotificationCheck(event) {
  ...
  .then((clientIsFocused) => {
    if (clientIsFocused) {
      console.log('Don\'t need to show a notification. not yet');
...
self.addEventListener('message', function(event) {
  ...
  switch(event.data) {
    ...
    case 'send-message-to-page-demo':
      ...
      break;
    case 'show-sw-version':
      console.log(serviceWorkerVersion);
      event.ports[0].postMessage(serviceWorkerVersion);
      break;
    default:
